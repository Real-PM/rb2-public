...
def team_detail(team_id):
    ...
    with timing_context('query_team'):
    ...
    with timing_context('query_coaches'):
    ...
    with timing_context('query_franchise_history'):
        franchise_history = get_franchise_history(team_id)

    with timing_context('query_franchise_top_players'):
        franchise_top_players = get_franchise_top_players(team_id, limit=24)

    with timing_context('query_franchise_year_by_year'):
...
def team_year(team_id, year):
    ...
    with timing_context('query_player_batting_stats'):
        player_batting_stats = get_team_player_batting_stats(team_id, year)

    with timing_context('query_player_pitching_stats'):
